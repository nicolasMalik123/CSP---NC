...
public class VertexPulse {
	...
	public void pulse(int[]pulseWeights, ArrayList<Integer> path) 
	{
		...
		if (this.firstTime) {
			...
			this.Sort(this.magicIndex); //Reviso primero al arco que tiene una menor distancia. (Menor costo)
			leftDist = null;
			rigthDist = null;
			reverseEdges = null;
		}
		...
		// Label update
		changeLabels(pulseWeights); //Cambia los labels (En el primer caso entran siendo cero).
			
		//Check the path completion:
		
		if(completePathCheck(pulseWeights,path)){
			// Check for cycles
			if (PulseGraph.Visited[id]==0) {   //Esto se basa en el principio de que pasar por encima de nuevo no ayuda en nada !! (Muy importante esto es para un camino determinado)
				// Add the node to the path
				path.add(id);
				// Update the visit indicator
				PulseGraph.Visited[id]=1;
				// Pulse all the head nodes for the outgoing arcs
				for (int i = 0; i < magicIndex.size(); i++) { //Recordemos que cojo primero al arco que tiene menor costo
					...
					// Pruning strategies: infeasibility, bounds and labels
						...
						//Podar por infactibilidad:
							//NewTime <= (PulseGraph.TimeC - PulseGraph.vertexes[DataHandler.Arcs[magicIndex.get(i)][1]].getMinTime()) 
							//Tiempo actual + El mejor tiempo que obtendria ! <= Limite de tiempo
					
						//Por por cota:
							//NewDist <= (PulseGraph.PrimalBound - PulseGraph.vertexes[DataHandler.Arcs[magicIndex.get(i)][1]].getMinDist())
							//Distancia actual + La mejor distancia que obtendria ! <= Cota primal (La mejor solución hasta ahora)
					
						//Por por dominancia
							//!PulseGraph.vertexes[DataHandler.Arcs[magicIndex.get(i)][1]].CheckLabels(NewTime, NewDist))
							//Es dominado si:
								//El tiempo y la distancia son mayores o iguales que los label actuales
					...
					if ( (newWeights[0] + PulseGraph.vertexes[a].getMinTime() <= PulseGraph.TimeC) && (newWeights[1] + PulseGraph.vertexes[a].getMinDist() < PulseGraph.PrimalBound) && !PulseGraph.vertexes[a].CheckLabels(newWeights,a)){
						...
						//Aca se mueve a dicho sucesor. 
						
					}
					//En caso de no haber cumplido se pasaria al siguiente sucesor
				}
				// Updates path and visit indicator for backtrack
				// Una vez ya se visito a todos los sucesores, es necesario devolverse
	...
	/**
	 * Path completion
	 * @param pulseWeights: [0] time [1] cost
	 * @param path: the partial path
	 * @return true if the search must go on, false if the pulse is stopped
	 */
	public boolean completePathCheck(int[] pulseWeights,ArrayList<Integer>path){
		if(pulseWeights[0]+ this.getMaxTime() <= PulseGraph.TimeC && pulseWeights[1] + this.getMinDist() < PulseGraph.PrimalBound){
			...
			PulseGraph.finalNode = id;
			PulseGraph.finalCost = pulseWeights[1];
			PulseGraph.finalTime = pulseWeights[0];
			PulseGraph.best = 1;
